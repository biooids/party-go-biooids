...
import { RefreshToken } from "../db/mongo.js";
import {
...
export const generateAccessToken = (user: {
  ...
  const options: SignOptions = {
    expiresIn: `${config.jwt.accessExpiresInSeconds}s`, // Best practice to include the unit
  };
...
export const verifyAndValidateRefreshToken = async (
  ...
  try {
    ...
    const storedToken = await RefreshToken.findOne({ jti: decoded.jti }).lean();
    ...
    if (storedToken.revoked) {
      logger.fatal(
        { userId: storedToken.userId, jti: decoded.jti },
        "CRITICAL: Attempted reuse of a revoked refresh token. All user sessions have been invalidated."
      );
      ...
      throw createHttpError(
        403,
        "Session has been compromised. All active sessions were terminated. Please log in again."
      );
    }
